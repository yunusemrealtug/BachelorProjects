...
def check_result (state, playerNum):
    ...
    if (state[2][1]==8 and state[2][2]==9) or (state[2][1]==9 and state[2][2]==8):
        ...
        return 0
#checks the win condition on the board    
...
def check_repeat (state_object, start_player):
    ...
    return 0
#checks if the opposent moves are repeating
                    ...
                    if find_position(state_object.state, 9)==(0,0):
                        if find_position(state_object.parent.parent.state, 9)==(0,0):
                            if find_position(state_object.parent.parent.parent.state, 9)==(0,0):
                                return 1 if state_object.player==start_player else -1
    return 0
# checks if the opposent is blocking the winning places
...
#finds the position of the given number in the board
...
#creates the tree of the game

def minMaxDecision (state_object, start_player):
    global extendedNodes
    extendedNodes+=1

    if state_object.value!=-2:
        return (state_object.value, state_object.maxDepth)
    
    if state_object.player==start_player:
        bestVal=-2
        bestDepth=-3
        for child in state_object.children:
            arr=minMaxDecision(child, start_player)
            val=arr[0]
            dep=arr[1]
            
            if bestVal==val:
                bestDepth=max(bestDepth, dep)
            elif bestVal<val:
                bestVal=val
                bestDepth=dep
            state_object.value=bestVal
            state_object.maxDepth=bestDepth
