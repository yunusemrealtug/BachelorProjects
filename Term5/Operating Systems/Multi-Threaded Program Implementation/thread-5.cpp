...
void* minAndMed(void* list) { //function for thread1
    minimum(list);
    median(list);
}

void* maxAndMode(void* list) { //function for thread2
    maximum(list);
    mode(list);
}
void* rangeAndIRange(void* list) { //function for thread3
    range(list);
    irange(list);
}
void* sumAndAmean(void* list) { //function for thread4
    sum(list);
    amean(list);
}
void* hmeanAndDeviation(void* list) { //function for thread5
    hmean(list); 
    deviation(list);
}
...
int main(int argc, char** argv) {
    ...
    std::ofstream MyFile("output2.txt"); //open new output2.txt
